...
{
    if (new_mu.size() != n_dofs_)
    {
        std::cerr << "Error: new_mu size mismatch in change_variable_prior" << std::endl;
        return;
    }
    ...
    for (auto &[fkey, fac] : factors_)
    {
        if (outbox_.find(fkey) != outbox_.end())
        {
            outbox_[fkey] = belief_;
        }
        if (inbox_.find(fkey) != inbox_.end())
        {
            inbox_[fkey].setZero();
        }
    }
